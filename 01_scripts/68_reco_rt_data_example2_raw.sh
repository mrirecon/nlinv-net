...
DIR=$RECODIR/21_recos_rt/$(basename $RAW)_slc13_raw/

mkdir -p $DIR/02_rtnlinv_rovir/
$SCRIPT_DIR/10_reco_rt.sh -R -s13 -S13 -V $RAW $DIR/02_rtnlinv_rovir/img_rt

NETWORK=$SCRIPT_DIR/../11_networks_rt/44_rovir_resnet_mad_first/
NET=$(basename $NETWORK)

mkdir -p $DIR/$NET
$SCRIPT_DIR/11_reco_nlinvnet_rt.sh -R -s13 $RAW $NETWORK $DIR/$NET/img
